...
friction_chamfer_depth = (outer_thickness - friction_space - thickness) * 2.0 # atan(1 / 2) angle
...
remove_sketch_plane = Plane(
    origin=end_face_center + (front_back_thickness, 0, 0), x_dir=(0, 1, 0), z_dir=(1, 0, 0)
)
...
def make_friction_style_part(thickness):
    part = Part()
    rect = Rectangle(
        total_length - thickness * 2,
        total_width - thickness * 2
    )
    face = (
        Rectangle(total_length, total_width)
        - rect
    )
...
buckle = fillet(buckle.edges() - buckle.edges().filter_by(Axis.X)
                   - buckle.edges().filter_by(Axis.Y), radius=buckle_fillet)
...
body = chamfer(body.edges().group_by(Axis.Z)[-1], length2=outer_thickness - friction_space - thickness, length=friction_chamfer_depth)
body -= remove_sketch_plane * air
buckle_front_air_plane = Plane(removed_friction_air.faces().filter_by(Axis.Y).sort_by(Axis.Y)[1]) # front inner
front_buckle_air = buckle_front_air_plane * Pos(Y=friction_depth / 2.0 - buckle_edge_distance) * buckle
...
upper_box_air = extrude(
    ...
    taper=math.degrees(math.atan((outer_thickness - friction_part_thickness) / (upper_depth - outer_thickness)))
)
...
buckle_front_mount_plane = Plane(friction_part.faces().filter_by(Axis.Y).sort_by(Axis.Y)[1]) # front inner
front_buckle = buckle_front_mount_plane * Pos(Y=friction_depth / 2.0 - buckle_edge_distance) * buckle
